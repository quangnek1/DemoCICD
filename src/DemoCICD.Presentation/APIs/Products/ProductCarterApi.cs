...
public class ProductCarterApi : ApiEndpoint, ICarterModule
{
    ...
    public static async Task<IResult> GetProductsV1(ISender sender, string? serchTerm = null,
        ...
        int pageSize = 10)
    {
        var result = await sender.Send(new CommandV1.Query.GetProductQuery(serchTerm, sortColumn,
    ...
    {
        var result = await sender.Send(new CommandV1.Command.DeleteProduct(productId));
        return Results.Ok(result);
    }

    public static async Task<IResult> UpdateProductsV1(ISender sender, Guid productId, [FromBody] CommandV1.Command.UpdateProduct updateProduct)
    {
        var updateProductCommand = new CommandV1.Command.UpdateProduct(productId, updateProduct.Name, updateProduct.Price, updateProduct.Description);
    ...
    {
        var result = await sender.Send(new CommandV2.Command.DeleteProduct(productId));
        return Results.Ok(result);
    }

    public static async Task<IResult> UpdateProductsV2(ISender sender, Guid productId, [FromBody] CommandV2.Command.UpdateProduct updateProduct)
    {
        var updateProductCommand = new CommandV2.Command.UpdateProduct(productId, updateProduct.Name, updateProduct.Price, updateProduct.Description);
        var result = await sender.Send(updateProductCommand);
        return Results.Ok(result);
    }
